...
client.on('data', function(data,err) { 
    ...
    winston.info('Reach client: ', currentLocation);
});
...
imu_client.on('end',()=>{
	    winston.info("ERROR: Imu disconnected");
});

client.on('end',()=>{
	    winston.info("ERROR: Reach disconnected");
});
//----END REACH CODE----
//----IMU ENTRY----
...
function calc_heading_delta(){
    ...
    winston.info('***calc_location: ', currentLocation);
    winston.info('***calc_reviouslocation: ',previousLocation); 
    ...
    if (invalidHeadingCounter >= 50 || previousLocation.longitude === currentLocation.longitude) {
        clearInterval(drive_timer);
        clearInterval(turn_timer);
        setTimeout(()=>{ 
        ...
        if( previousLocation.longitude === currentLocation.longitude){  
            winston.info('ERROR: same location detected');
        }
        else{
            winston.info('ERROR: invalid heading');
        }
    }
