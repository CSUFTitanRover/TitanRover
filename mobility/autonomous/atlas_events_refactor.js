...
//DRIVE-CONSTANTS: 
var turning_drive_constant = 35; //Turning Motor Constant
var forward_drive_constant = 30; //ForwardP Motor Constant
var forward_drive_modifier = 1; //to modify when driving straight forward
...
var forward_throttle_max = 45; //Maximum throttle value acceptable
...
var turn_right = null;
var onTarget = false;
var badHeadingData = false;
var badLocationData = false;
...
client.on('data', function(data,err) { 
    ...
    if (badLocationData && (previousLocation.lattitude != currentLocation.lattitude || previousLocation.longitude != currentLocation.longitude)) { 
        badLocationData = false;
    }
...
atlas.on('turn',function(){
    ...
    */
    turn_timer = setInterval(function() {
        ...
        if (badHeadingData || badLocationData) {
            winston.info("ERROR: Bad Data Stopping Rover")
            winston.info("badHeadingData: ", badHeadingData);
            winston.info("badLocationData: ", badLocationData);
            stopRover();
    ...
    */
    drive_timer = setInterval(function() {
        ...
        // Adjust left and right throttle to maintain nominal heading
            ...
            throttle_offset = 2 * Math.round(forward_drive_constant * (Math.pow(Math.E, y)));
